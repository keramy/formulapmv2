...
BEGIN
  RETURN EXISTS (
    SELECT 1 FROM user_profiles 
    WHERE id = auth.uid() 
    AND role IN ('management', 'admin', 'project_manager', 'technical_lead')
  );
END;
$$ LANGUAGE plpgsql SECURITY DEFINER;
...
BEGIN
  RETURN EXISTS (
    SELECT 1 FROM user_profiles 
    WHERE id = auth.uid() 
    AND role IN ('management', 'admin', 'project_manager', 'purchase_manager')
  );
END;
$$ LANGUAGE plpgsql SECURITY DEFINER;
...
CREATE POLICY "Requester own purchase request access" ON purchase_requests
  FOR ALL USING (
    requester_id = auth.uid() AND (
      status = 'draft' OR 
      EXISTS (SELECT 1 FROM user_profiles WHERE id = auth.uid() AND role IN ('project_manager', 'technical_lead'))
    )
  );
