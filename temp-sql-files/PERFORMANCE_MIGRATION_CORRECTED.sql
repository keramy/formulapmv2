-- =====================================================
-- Formula PM v2 - Performance Optimization Migrations (CORRECTED)
-- Target: Local Supabase Development Instance
-- Fixed for current user_profiles schema (id column, not user_id)
-- =====================================================
...
-- =====================================================
-- PART 2: SIMPLIFIED RLS POLICIES (Fixed for current schema)
-- =====================================================
...
-- Create simplified optimized policies using correct column names
-- Using 'id' column from user_profiles (not 'user_id')

-- Optimized scope items policy
CREATE POLICY scope_items_select_policy_optimized ON scope_items
FOR SELECT USING (
  -- Direct role check with indexed lookup (using correct column names)
  EXISTS (
    ...
    AND (
      ...
      OR
      -- Technical roles can see scope items for their projects
      (up.role IN ('technical_director', 'project_manager', 'architect', 'technical_engineer') 
       AND scope_items.project_id IN (
         SELECT id FROM projects 
         WHERE technical_lead_id = up.id 
         OR project_manager_id = up.id
       ))
    )
  )
);
...
FOR SELECT USING (
  ...
  EXISTS (
    ...
    AND (
      ...
      (up.role IN ('technical_director', 'project_manager', 'architect', 'technical_engineer') 
       AND (projects.technical_lead_id = up.id OR projects.project_manager_id = up.id))
      OR
...
FOR SELECT USING (
  -- Direct user access or role-based access (using correct column names)
  ...
  EXISTS (
    ...
    AND (
      ...
      up.role IN ('company_owner', 'general_manager', 'deputy_general_manager', 'admin')
      OR
      -- Technical roles can see tasks for their projects
      (up.role IN ('technical_director', 'project_manager', 'architect', 'technical_engineer') 
...
-- Test current user profile structure
