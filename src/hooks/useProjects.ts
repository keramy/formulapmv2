...
export const useProjects = () => {
  ...
  const createProject = useCallback(async (projectData: ProjectFormData) => {
    ...
    try {
      ...
      const token = await getAccessToken()
      if (!token) {
        console.error('❌ [createProject] No access token available - user may not be authenticated')
      ...
      if (!response.ok) {
        ...
        try {
          ...
          console.error('❌ [createProject] Failed to parse error response', parseError)
