...
export const POST = withAuth(async (request: NextRequest, { user, profile }, { params }) => {
  try {
    ...
    if (!allowedStatuses.includes(drawing.status)) {
      return createErrorResponse('Drawing cannot be approved in current status', 400)
    }

    // Determine next status based on review type and current status
    ...
    console.error('Shop drawing approval error:', error)
