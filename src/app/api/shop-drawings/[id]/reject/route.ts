...
export const POST = withAuth(async (request: NextRequest, { user, profile }, { params }) => {
  try {
    ...
    if (!allowedStatuses.includes(drawing.status)) {
      return createErrorResponse('Drawing cannot be rejected in current status', 400)
    }

    // Update drawing status to rejected
    ...
    console.error('Shop drawing rejection error:', error)
...
}, { permission: 'shop_drawings.reject' })