/**
 * SECURE VERSION - reports API Route
...
export const GET = withAuth(async (request: NextRequest, { user, profile }) => {
  try {
    ...
    if (!hasPermission(profile.role, 'reports.read')) {
...
export const POST = withAuth(async (request: NextRequest, { user, profile }) => {
  try {
    // Permission check for new 5-role structure
    if (!hasPermission(profile.role, 'reports.create')) {
      return NextResponse.json(
        { error: 'Insufficient permissions' },
        { status: 403 }
      )
  ...
  permission: 'reports.create' // Optional: specify required permission
})